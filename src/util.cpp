...
const char * const BITCOIN_CONF_FILENAME = "poutinecoin.conf";
const char * const BITCOIN_PID_FILENAME = "poutinecoin.pid";
...
#else
    const char* pszModule = "poutinecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Poutinecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Poutinecoin";
#else
    // Unix
    return pathRet / ".poutinecoin";
#endif
#endif
}
