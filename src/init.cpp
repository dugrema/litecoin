...
void Shutdown()
{
    ...
    RenameThread("poutinecoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/poutinecoin-project/poutinecoin>";
    const std::string URL_WEBSITE = "<https://poutinecoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("poutinecoin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if poutinecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if poutinecoin is started while in a temporary directory.\n",
